 ...
 *
 * @package   Charitable Windcave/Classes/Charitable_Windcave_Webhook_Processor
 * @author    Eric Daams
...
if ( ! class_exists( 'Charitable_Windcave_Webhook_Processor' ) ) :

	/**
	 * Charitable_Windcave_Webhook_Processor
	 *
	 * @since 1.0.0
	 */
	class Charitable_Windcave_Webhook_Processor {
		...
		/**
		 * Process an incoming Windcave IPN.
		 *
		 * @since  1.0.0
		 *
		 * @return void
		...
		public static function process() {
			...
			if ( ! $webhook ) {
				status_header( 500 );
				die( __( 'Invalid Windcave event.', 'charitable-windcave' ) );
			}

			$processor = new Charitable_Windcave_Webhook_Processor( $webhook );
			$processor->run();
		}
		...
		public function run() {
			try {
				 ...
				 * @since 1.0.0
				 *
				 * @param array $processors Array of Windcave event types and associated callback functions.
				 */
				$default_processors = apply_filters(
					'charitable_windcave_default_webhook_event_processors',
					[]
				);
				 ...
				 * @param ?      $webhook    The webhook object.
				 */
				do_action( 'charitable_windcave_webhook_event', $webhook_event, $this->webhook );
				...
				die(
					sprintf(
						...
						__( 'Webhook processing error: %s', 'charitable-windcave' )
					)
				);
			}
		}
